				...
				for(int n=0;n<ff;n++){
					...
					if(fl[ff].verti){
						...
						if(Math.signum(y-fl[ff].y)==fl[ff].dire){
							double dist1 = Math.hypot(y-fl[n].y,fl[ff].x-fl[n].x);
							double dist2 = Math.abs(y-fl[ff].y);
						...
						if(Math.signum(y-fl[n].y)==fl[n].dire){
							double dist1 = Math.hypot(y-fl[ff].y,fl[n].x-fl[ff].x);
							double dist2 = Math.abs(y-fl[n].y);
							//System.out.println(y+" "+dist1+" "+dist2);
							if(dist1==dist2){
					...
					double dpc = Math.hypot(fl[n].x-xx,fl[n].y-yy);
					double dpc1 = Math.hypot(fl[ff].x-xx,fl[ff].y-yy);
					if(fl[n].dire!=Math.signum(xx-fl[n].x))continue;
					//System.out.println("entrooo "+n+" "+ff+" 111");
					if(fl[ff].dire!=Math.signum(xx-fl[ff].x))continue;
					if(n==10 && ff==34)
						System.out.println("entrooo "+n+" "+ff+" 222 "+dpc+" "+dpc1);
...
class Inter implements Comparable{
	...
	public Inter(double t,double x,double y){
		...
		tmp = Math.floor(tmp);
		this.t = tmp/val;
		tmp = Math.floor(x*val);
		this.x = tmp/val;
		tmp = Math.floor(y*val);
		this.y = tmp/val;
	}
...
class Tal{
	...
	public Tal(double a[]){
		...
		BigDecimal a0,a1,a2,a3;
		a0 = dec(a[0]);
		a1 = dec(a[1]);
		a2 = dec(a[2]);
		a3 = dec(a[3]);
		this.m = dou((a3.subtract(a1)).divide(a2.subtract(a0),MathContext.DECIMAL32));
		//this.b = a[1]-m*a[0];
		this.b = dou(a1.subtract(dec(m).multiply(a0)));
		dire = a[2]>a[0]?1:((a[2]==a[0])?0:-1);
		//System.out.println(this.m+"*x+"+this.b+" "+dire);
	}
	public BigDecimal dec(double d){return BigDecimal.valueOf(d);}
	public double dou(BigDecimal d){return d.doubleValue();}
	
	public double dist(Tal a){
		double val = Math.pow(10,9);
		return Math.floor(val*Math.hypot(x-a.x,y-a.y))/val;
	}
}
